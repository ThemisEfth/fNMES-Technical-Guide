...
Created on Fri Sep 11 17:23:16 2020
...
# to find the address of the serial port,
# open terminal, and type: ls /dev/*
# should be something like: /dev/cu.usbmodem146301
...
ser1 = serial.Serial("/dev/tty.usbmodem1464101" , 19200, timeout=0, writeTimeout=0)

time.sleep(2) # wait 2 s

ser1.write(b'\xFF') # Start device

ser2 = serial.Serial("/dev/tty.usbmodem1464201", 19200, timeout=0, writeTimeout=0) 
...
Blink = Button(window, text="Blink", bg="white", fg="black", width=22, height=2,justify='center',wraplength=120, command=blink)
...
LED_OFF = Button(window, text="Screen Off", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120,command=LEDOFF)
...
LED_ON = Button(window, text="Screen On", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120,command=LEDON)
...
Set_Count_0 = Button(window, text="Reset Count", bg="white", fg="black",width=22, height=2,justify='center',font="bold",wraplength=120,command=Train_Reset)
...
Send_settings = Button(window, text="Start DAC1",bg="green", fg="black",font="bold",width=22, height=2, command=Start_stim_DAC1)

Send_settings.grid(column=1, row=12)   

Send_settings = Button(window, text="Start DAC2",bg="green", fg="black",font="bold",width=22, height=2, command=Start_stim_DAC2)

Send_settings.grid(column=2, row=12)   

Send_settings = Button(window, text="Start DACS",bg="green", fg="black",font="bold",width=22, height=2, command=Start_stim_DAC1_2)
...
Close_Port = Button(window, text="Close Port", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120,command=Close_COM)
...
Bipolar_En = Button(window, text="Bipolar (OpAmp Enabled)", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120,command=Bipolar)
...
Mono_EN = Button(window, text="Monopolar (OpAmp Enabled)", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120,command=MonopolarEN)
...
Set_Message = Label(window, text="Settings", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)

Set_Message.grid(column=0, row=2)



Input_User_Message = Label(window, text="DAC 1 Values", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)

Input_User_Message.grid(column=1, row=2)

Input_User_Message = Label(window, text="DAC 2 Values", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)
...
def Pulses_DAC1():
    ...
    HB_p = Pulse_no//256 # Integer (floor division) divided by high byte for 'Pulse Repetition Time' (256)
...
P_Message = Label(window, text="Number of Pulses (1-9999)", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)
...
#   Create integer for high and low byte
    ...
    HB_ov = int(OV//128) # Input (floor division) divided by high byte for 'Output Voltage' (128 mV) and convert to int

    LB_ov = int((OV%128)/0.5) # Calculate remainder (modulo division), then divide by low byte (0.5)) and convert to int
...
OV_Message = Label(window, text="Output Voltage (mV)", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)
...
OV_Input2 = Entry(window, bg="white", fg="black",font="bold", justify='center')
...
#   Create integer for highbyte and lowbyte
    ...
    PDms_hb = int(PD_ms//25.6) # Integer (floor division) divided by high byte for high byte (26.6 ms)

    PDms_lb = int(round(((PD_ms%25.6)/.1),2)) # Input divide (modulo division) low byte (0.1) then conv to int

    

#   Create high and low byte for serial command    

    PDms_HB = (PDms_hb).to_bytes(1, byteorder="little") # Convert integer to byte

    PDms_LB = (PDms_lb).to_bytes(1, byteorder="little") # Convert integer to byte
...
PP_Message = Label(window, text="Pulse Delay (ms)", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)

PP_Message.grid(column=0, row=5)



PP_Input = Entry(window, bg="white", fg="black",width=20, font="bold", justify='center')
...
PP_Input2 = Entry(window, bg="white", fg="black",width=20, font="bold", justify='center')
...
##Set Pulse Width (microiseconds) DAC1

def Pulse_Width_us_DAC1():

    PW_us = int(PWus_Input.get()) # Get user input

    

#   Create integer for highbyte

    PWus_hb = PW_us//256 # Integer (floor division) divided by high byte for repeition width (256 ms)

    PWus_lb = int((PW_us%256)/1) # Take user input modulo division by 25.6 and divide by 1 then convert to int

    

#   Create high and low byte for serial command    

    PWus_HB = (PWus_hb).to_bytes(1, byteorder="little") # Convert integer to byte

    PWus_LB = (PWus_lb).to_bytes(1, byteorder="little") # Convert integer to byte
...
PWus_Message = Label(window, text="Pulse Width (μs)", bg="white", fg="black",width=22, height=2,justify='center',wraplength=120)

PWus_Message.grid(column=0, row=8)



PWus_Input = Entry(window, bg="white", fg="black",font="bold", justify='center')
...
def Pulse_Width_us_DAC2():
    ...
    PW_us2 = int(PWus_Input2.get()) # Get user input

    

#   Create integer for highbyte

    PWus_hb2 = PW_us2//256 # Integer (floor division) divided by high byte for repeition width (256 ms)

    PWus_lb2 = int((PW_us2%256)/1) # Take user input modulo division by 25.6 and divide by 1 then convert to int

    

#   Create high and low byte for serial command    

    PWus_HB2 = (PWus_hb2).to_bytes(1, byteorder="little") # Convert integer to byte

    PWus_LB2 = (PWus_lb2).to_bytes(1, byteorder="little") # Convert integer to byte
...
#   Send Serial Command for Repetition Time (39)  
    ...
    if PWus_hb2 > 0 or PWus_lb2 > 0: 
        ...
        ser2.write(PWus_HB2) # Set Highbyte

        ser2.write(PWus_LB2) # Set Lowbyte
...
PWus_Input2 = Entry(window, bg="white", fg="black",font="bold", justify='center')
...
Send_settings = Button(window, text="Set DAC1",bg="white", fg="black",font="bold", width=22, height=2, command=Send)
...
Send_settings = Button(window, text="Set DAC2",bg="white", fg="black",font="bold", width=22, height=2, command=Send)
...
Send_settings = Button(window, text="Set DAC1 & DAC2",bg="white", fg="black",font="bold", width=22, height=2, command=Send_DACS)
